    ...
    module2.exports = function(iconv) {
      ...
      iconv.extendNodeEncodings = /* @__PURE__ */ __name(function extendNodeEncodings() {
        ...
        if (iconv.supportsStreams) {
          ...
          Readable.prototype.setEncoding = /* @__PURE__ */ __name(function setEncoding(enc, options) {
...
var require_lib = __commonJS({
  "node_modules/iconv-lite/lib/index.js"(exports, module2) {
    ...
    iconv.encode = /* @__PURE__ */ __name(function encode(str, encoding, options) {
      ...
      return trail && trail.length > 0 ? Buffer2.concat([
        res,
        trail
      ]) : res;
    }, "encode");
...
var require_urlencoded = __commonJS({
  "node_modules/body-parser/lib/types/urlencoded.js"(exports, module2) {
    ...
    function urlencoded(options) {
      ...
      __name(parse, "parse");
      return /* @__PURE__ */ __name(function urlencodedParser2(req, res, next) {
        if (req._body) {
          debug("body already parsed");
          next();
          return;
...
var require_body_parser = __commonJS({
  "node_modules/body-parser/index.js"(exports, module2) {
    ...
    var deprecate = require_depd()("body-parser");
    var parsers = /* @__PURE__ */ Object.create(null);
    exports = module2.exports = deprecate.function(bodyParser3, "bodyParser: use individual json/urlencoded middlewares");
    ...
    function bodyParser3(options) {
      ...
      return /* @__PURE__ */ __name(function bodyParser4(req, res, next) {
...
// src/connexion_bdd.ts
var mysql = __toESM(require("mysql2"));
var module_connexion = mysql.createConnection({
  host: "localhost",
  port: 3306,
  user: "root",
  password: "root",
...
// src/resources/professeur/prof.service.ts
function findProf(id, pwd, req, res) {
  let session2 = req.session;
  if (id == 0 || pwd == "" || id == void 0 || pwd == void 0) {
    req.session.message = "Identifiant / mot de passe vide invalide ";
    console.log(session2.message);
  } else {
    module_connexion.query("SELECT idProfesseur, nomProfesseur, prenomProfesseur FROM Professeur WHERE idProfesseur = ? AND password = SHA1(?)", [
      id,
      pwd
    ], (err, result, fields) => {
      if (err) {
        session2.message = "err" + err.message;
      } else {
        if (result.toString() == "") {
          session2.message = "Authentification incorrecte";
        } else {
          session2.userid = result;
          console.log(result);
        }
      }
    });
...
__name(findProf, "findProf");
function getMatiereProf(idProf, req, res) {
  let session2 = req.session;
  module_connexion.query("SELECT m.idMatiere, libelle FROM Matiere m, Prof_Matiere pm WHERE pm.idMatiere = m.idMatiere AND pm.idProfesseur = ?", [
    idProf
  ], (err, result, fields) => {
    if (err) {
      session2.error = err.message.toString();
    } else {
      if (result.toString() == "") {
        session2.error = "Pas de donn\xE9es";
      } else {
        session2.matiereProf = result;
        console.log(session2);
      }
    }
  });
}
__name(getMatiereProf, "getMatiereProf");
function getClasseProf(idProf, req, res) {
  let session2 = req.session;
  module_connexion.query("SELECT idSection, libelleSection FROM Section WHERE idProfesseur = ?", [
    idProf
  ], (err, result, fields) => {
    if (err) {
      session2.error = err.message.toString();
    } else {
      session2.classeProf = result;
    }
  });
}
__name(getClasseProf, "getClasseProf");

// src/resources/professeur/prof.ctrl.ts
function logout(req, res) {
  let session2 = req.session;
  session2.destroy(function(err) {
...
function accueil(req, res) {
  ...
  findProf(id, pwd, req, res);
  let session2 = req.session;
  let messsage = req.session.message;
  res.locals.message = messsage;
  if (typeof session2.userid === "undefined") {
    console.log("CA MARCHE PAS");
    res.redirect("/");
  } else {
    getMatiereProf(session2.userid[0]["idProfesseur"], req, res);
    getClasseProf(session2.userid[0]["idProfesseur"], req, res);
    if (typeof session2.error == "undefined") {
      console.log(session2.error);
      res.send(session2.error);
    } else {
      console.log(session2);
      res.render("accueil", {
        user: session2.userid[0]["nomProfesseur"] + " " + session2.userid[0]["prenomProfesseur"],
        matiere: session2.matiereProf,
        classe: session2.classeProf
      });
    }
  }
}
__name(accueil, "accueil");

// src/routes/prof_route.ts
var express = __toESM(require("express"));
var router_prof = express.Router();
var bodyParser = require_body_parser();
var urlencodedParser = bodyParser.urlencoded({
  extended: true
});
router_prof.post("/accueil", urlencodedParser, accueil);
router_prof.get("/logout", logout);

// src/index.ts
var import_cookie_parser = __toESM(require("cookie-parser"));
var session = require("express-session");
var urlencodedparser = import_body_parser.default.urlencoded({
  extended: true
});
var app = (0, import_express.default)();
app.set("view engine", "ejs");
app.set("views", import_path.default.join(__dirname, "views"));
app.set("public", import_path.default.join(__dirname, "public"));
